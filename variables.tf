...
display_name: (Required If need to provision) The display name of the policy definition.
description: (Optional, default `""`) The description of the policy definition.
type: (Optional, default `Custom`) The policy type.
mode: (Optional, default `All`) The policy resource manager mode that allows you to specify which resource types will be evaluated.
metadata: (Optional) The metadata for the policy definition.
...
specific_role_definition_ids: (Optional) List of Explicit Role definition ID, used in Role assignments for Remediation
EOF
    type        = any
    default     = []
}
...
type: (Optional, default `Custom`) The Initiative type.
